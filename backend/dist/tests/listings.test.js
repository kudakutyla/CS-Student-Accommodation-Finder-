...
describe('Sprint 2: Listings, Approval Workflow & Search Tests', () => {
    ...
    beforeAll(async () => {
        ...
        const landlordRes = await (0, supertest_1.default)(app_1.default).post('/api/auth/login').send({
            email: 'landlord@finder.co.za',
            password: 'LandlordPass123!',
        });
        verifiedLandlordToken = landlordRes.body.data.token;
        const campusRes = await (0, supertest_1.default)(app_1.default).get('/api/campuses');
    ...
    describe('Listing Approval Workflow & Public Visibility', () => {
        it('Pending listing MUST NOT appear in public GET /api/listings', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get('/api/listings');
            expect(res.status).toBe(200);
            const found = res.body.data.items.find((item) => item.id === newListingId);
        ...
        it('Admin can view the pending listing in the approval queue', async () => {
            ...
            expect(res.status).toBe(200);
            const found = res.body.data.find((item) => item.id === newListingId);
        ...
        it('Admin rejects listing with reason -> status becomes REJECTED and AuditLog created', async () => {
            ...
            expect(res.body.data.rejectionReason).toBe('Incomplete compliance documentation regarding fire safety.');
            // Now it must disappear from public search again
            const publicRes = await (0, supertest_1.default)(app_1.default).get('/api/listings');
            const found = publicRes.body.data.items.find((item) => item.id === newListingId);
            expect(found).toBeUndefined();
        });
    });
    describe('Student Search, Filters, Sorting & Pagination', () => {
        it('Filters by price range correctly', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get('/api/listings?minPrice=3000&maxPrice=5000');
            expect(res.status).toBe(200);
        ...
        it('Sorts by price ascending', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get('/api/listings?sort=price-asc');
            expect(res.status).toBe(200);
        ...
        it('Sorts by nearest distance from campus', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get('/api/listings?sort=nearest');
            expect(res.status).toBe(200);
            const items = res.body.data.items;
            for (let i = 0; i < items.length - 1; i++) {
        ...
        it('Supports pagination with page, limit, and total count metadata', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get('/api/listings?page=1&limit=2');
            expect(res.status).toBe(200);
        ...
        it('Provides full listing detail on GET /api/listings/:id', async () => {
            ...
            const listRes = await (0, supertest_1.default)(app_1.default).get('/api/listings');
            const sample = listRes.body.data.items[0];
            const detailRes = await (0, supertest_1.default)(app_1.default).get(`/api/listings/${sample.id}`);
