...
async function createListing(req, res) {
    try {
        ...
        if (req.user.role !== 'LANDLORD') {
            (0, response_1.sendError)(res, 'Only landlords can create accommodation listings.', 403);
            return;
        }
        // Verification check
        ...
        const parsed = listing_validator_1.createListingSchema.safeParse(req.body);
        if (!parsed.success) {
            (0, response_1.sendError)(res, parsed.error.errors[0].message, 400, parsed.error.format());
            return;
        }
...
async function getMyListings(req, res) {
    try {
        ...
        (0, response_1.sendSuccess)(res, { items: formatted, stats });
    }
    catch (error) {
        console.error('getMyListings error:', error);
        (0, response_1.sendError)(res, 'Failed to fetch landlord listings', 500);
    }
}
async function getPublicListings(req, res) {
    try {
        const parsed = listing_validator_1.listingSearchQuerySchema.safeParse(req.query);
        if (!parsed.success) {
        ...
        if (sort === 'price-asc')
            ...
            orderBy = { distanceFromCampus: 'asc' };
        else if (sort === 'newest')
            orderBy = { createdAt: 'desc' };
        ...
        (0, response_1.sendError)(res, 'Failed to search listings', 500);
    }
}
async function getListingById(req, res) {
    try {
        ...
        (0, response_1.sendError)(res, 'Failed to fetch listing details', 500);
    }
}
async function updateListing(req, res) {
    try {
        ...
        if (existing.ownerId !== req.user.id && req.user.role !== 'ADMIN') {
            (0, response_1.sendError)(res, 'Forbidden: You can only modify your own listings', 403);
            return;
        }
        const parsed = listing_validator_1.updateListingSchema.safeParse(req.body);
        if (!parsed.success) {
            (0, response_1.sendError)(res, parsed.error.errors[0].message, 400);
            return;
        }
        ...
        (0, response_1.sendSuccess)(res, updated, 'Listing updated successfully');
    }
    catch (error) {
...
async function deleteListing(req, res) {
    try {
        ...
        // Ownership check
        if (existing.ownerId !== req.user.id && req.user.role !== 'ADMIN') {
            (0, response_1.sendError)(res, 'Forbidden: You can only delete your own listings', 403);
            return;
        }
        await prisma_1.default.listing.delete({
            where: { id },
        });
        (0, response_1.sendSuccess)(res, null, 'Listing deleted successfully');
    }
    catch (error) {
        console.error('deleteListing error:', error);
        (0, response_1.sendError)(res, 'Failed to delete listing', 500);
    }
}
